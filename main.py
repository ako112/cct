    ...
    with open("live_ipv4.m3u", "w", encoding="utf-8") as f_m3u4, \
        ...
        for category, names in template.items():
            ...
            for name in names:
                ...
                # 直接写入原始URL
                ipv4_urls = [u.split('$', 1)[0] if '$' in u else u for u in channels[category][name] if not is_ipv6(u)]
                for idx, url in enumerate(sort_urls(ipv4_urls, ipv4_seen), 1):
                    write_entry(f_m3u4, f_txt4, category, name, url, idx)
                
                ipv6_urls = [u.split('$', 1)[0] if '$' in u else u for u in channels[category][name] if is_ipv6(u)]
